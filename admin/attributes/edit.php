...
$attributeId = isset($_GET['id']) ? (int)$_GET['id'] : 0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && validateCSRF($_POST['csrf_token'])) {
    ...
    if (!empty($name)) {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM attributes WHERE name = ? AND id != ?");
        $stmt->execute([$name, $attributeId]);
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("UPDATE attributes SET name = ?, description = ?, type = ?, default_value = ?, options = ?, is_required = ?, is_active = ?, updated_at = NOW() WHERE id = ?");
            $stmt->execute([$name, $description, $type, $default_value, $options, $is_required, $is_active, $attributeId]);
            ...
            $errors[] = "Error updating attribute: " . $e->getMessage();
        }
    }
}
...
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-3xl font-bold text-gray-800">Edit Attribute</h1>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select id="type" name="type" required 
                    ...
                    <option value="text" <?php echo $attribute['type'] === 'text' ? 'selected' : ''; ?>>Text</option>
                    <option value="number" <?php echo $attribute['type'] === 'number' ? 'selected' : ''; ?>>Number</option>
                    <option value="select" <?php echo $attribute['type'] === 'select' ? 'selected' : ''; ?>>Select</option>
                    <option value="radio" <?php echo $attribute['type'] === 'radio' ? 'selected' : ''; ?>>Radio</option>
                    <option value="checkbox" <?php echo $attribute['type'] === 'checkbox' ? 'selected' : ''; ?>>Checkbox</option>
                    <option value="boolean" <?php echo $attribute['type'] === 'boolean' ? 'selected' : ''; ?>>Boolean</option>
                </select>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="description" name="description" rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"><?php echo htmlspecialchars($attribute['description']); ?></textarea>
            </div>
            ...
            <div>
                ...
                <input type="text" id="default_value" name="default_value" 
                       value="<?php echo htmlspecialchars($attribute['default_value']); ?>"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>

            <div id="options-field" style="display: <?php echo in_array($attribute['type'], ['select', 'radio', 'checkbox']) ? 'block' : 'none'; ?>;">
                ...
                <textarea id="options" name="options" rows="3" 
                          ...
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"><?php echo htmlspecialchars($attribute['options']); ?></textarea>
            ...
            <div class="flex items-center">
                <input type="checkbox" id="is_active" name="is_active" value="1" 
                       <?php echo $attribute['is_active'] ? 'checked' : ''; ?>
                       class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
