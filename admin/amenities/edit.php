...
$amenityId = isset($_GET['id']) ? (int)$_GET['id'] : 0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && validateCSRF($_POST['csrf_token'])) {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("UPDATE amenities SET name = ?, description = ?, icon = ?, is_active = ?, updated_at = NOW() WHERE id = ?");
            $stmt->execute([$name, $description, $icon, $is_active, $amenityId]);
            
            $success = "Amenity updated successfully!";
            
            // Refresh amenity data
            $stmt = $pdo->prepare("SELECT * FROM amenities WHERE id = ?");
            $stmt->execute([$amenityId]);
            $amenity = $stmt->fetch();
        } catch (PDOException $e) {
            $errors[] = "Error updating amenity: " . $e->getMessage();
        }
    }
}
...
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-3xl font-bold text-gray-800">Edit Amenity</h1>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" id="name" name="name" required 
                       value="<?php echo htmlspecialchars($amenity['name']); ?>"
            ...
            <div>
                ...
                <input type="text" id="icon" name="icon" 
                       value="<?php echo htmlspecialchars($amenity['icon']); ?>"
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"><?php echo htmlspecialchars($amenity['description']); ?></textarea>
            </div>
    ...
    <div class="mt-6 bg-gray-50 p-4 rounded-lg">
        ...
        <div id="icon-preview" class="text-2xl text-gray-400">
            <?php if ($amenity['icon']): ?>
                ...
                <span class="text-sm ml-2"><?php echo htmlspecialchars($amenity['icon']); ?></span>
            <?php else: ?>
